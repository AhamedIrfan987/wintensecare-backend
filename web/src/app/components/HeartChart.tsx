...
import { getTickCount } from "@/lib/utils/range";
...
export default function HeartChart({
  ...
  const minHeartRate = Math.min(...points.map(p => p.heartRate), 40);
  const maxHeartRate = Math.max(...points.map(p => p.heartRate), 160);
  ...
  // Calculate heart rate variability if we have enough points
  const hrv = points.length > 1 
    ? Math.sqrt(points.reduce((acc, p, i) => {
        if (i === 0) return acc;
        const diff = p.heartRate - points[i-1].heartRate;
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <XAxis
            ...
            tickCount={getTickCount(range as any)}
            tick={{ 
          ...
          {summary && (
            <>
              <ReferenceLine
                ...
                strokeWidth={1.5}
                label={{
                  value: `Min: ${summary.minHeartRate}`,
                  position: 'right',
                  fill: theme.palette.success.main,
                  fontSize: 10,
                }}
              ...
              <ReferenceLine
                ...
                strokeWidth={1.5}
                label={{
                  value: `Avg: ${summary.avgHeartRate}`,
                  position: 'right',
              ...
              <ReferenceLine
                ...
                strokeWidth={1.5}
                label={{
                  value: `Max: ${summary.maxHeartRate}`,
                  position: 'right',
                  fill: theme.palette.error.main,
                  fontSize: 10,
                }}
          ...
          {/* Legend */}
          {showStats && <Legend content={<CustomLegend summary={summary} />} />}
          ...
          {/* Brush for range selection on larger time ranges */}
          {range === "7d" && !isMobile && (
            <Brush
              dataKey="ts"
              height={30}
